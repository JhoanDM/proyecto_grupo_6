...
class Aplicacion:
    ...
    def agregar_componentes(self):
        ...
        self.entry1 = ttk.Entry(self.labelframe1, width=15, textvariable=self.dato1)
        ...
        self.entry2 = ttk.Entry(self.labelframe1, width=15, textvariable=self.dato2)
    ...
    def sumar(self):
            if self.dato1.get()=="" or self.dato2.get()=="":
                mb.showerror("Cuidado", "No puede dejar los cuadros de entrada vacíos")
            else:
                SUMA=int(self.dato1.get())+int(self.dato2.get())
                self.ventana1.title(f"La suma es {SUMA}")

    def acerca(self):
            mb.showinfo("Información", " Este Programa fue desarrollado para el aprendizaje de python")
